...
`actions/configure-pages` configures the GitHub Pages deployment and exposes the site's `base_url` as a step output. The `id: pages` is required so the output can be referenced in the next step.
